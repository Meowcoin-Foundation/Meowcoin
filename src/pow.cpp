...
{
    assert(pindexLast != nullptr);
    const int64_t T = params.nPowTargetSpacing * 2; // 2 for number of algorithms (meowpow + scrypt)
    ...
    for (int64_t i = N; i > 0; i--) {
        ...
        thisTimestamp = block->GetBlockTime() - block_Prev->GetBlockTime();
