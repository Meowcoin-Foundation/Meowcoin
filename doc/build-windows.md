...
Then build using:
    ...
    CONFIG_SITE=$PWD/depends/x86_64-w64-mingw32/share/config.site ./configure --prefix=/ --enable-sse2 --disable-tests
    make
...
Then build using:
    ...
    cd ..
    ./autogen.sh # not required when building from tarball
    CONFIG_SITE=$PWD/depends/i686-w64-mingw32/share/config.site ./configure --prefix=/ --enable-sse2 --disable-tests
    make
...
way. This will install to `c:\workspace\meowcoin`, for example:
    ...
    make install DESTDIR=/mnt/c/workspace/meowcoin
